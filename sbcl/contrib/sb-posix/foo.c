...
{ struct stat t;
    ...
    fprintf (out, "))\n");
    fprintf (out, "(cl:eval-when (:compile-toplevel :load-toplevel :execute) (sb-grovel::define-c-struct ALIEN-TIMESPEC %d\n", sizeof(struct timespec));
    fprintf (out, " (TV-SEC TIME-T \"time_t\"\n");
{ struct timespec t;
...
{ struct timespec t;
    ...
    fprintf (out, " (TV-NSEC LONG \"long\"\n");
{ struct timespec t;
    fprintf (out, "  %d\n", ((unsigned long)&(t.tv_nsec)) - ((unsigned long)&(t)));
}
{ struct timespec t;
    fprintf (out, "  %d)\n", sizeof(t.tv_nsec));
}
