...
{ struct in_addr t;
    ...
    fprintf (out, " (LEN (UNSIGNED 8) \"__uint8_t\"\n");
{ struct sockaddr_in t;
    fprintf (out, "  %d\n", ((unsigned long)&(t.sin_len)) - ((unsigned long)&(t)));
}
{ struct sockaddr_in t;
    fprintf (out, "  %d)\n", sizeof(t.sin_len));
}
...
{ struct msghdr t;
    ...
    fprintf (out, "  (* HOSTENT)\n  (NAME C-STRING))\n");
    fprintf (out, "(cl:declaim (cl:inline GETHOSTBYNAME2))\n");
    fprintf (out, "(sb-grovel::define-foreign-routine (\"gethostbyname2\" GETHOSTBYNAME2)\n");
    fprintf (out, "  (* HOSTENT)\n  (NAME C-STRING)\n  (AF INT))\n");
...
{ struct addrinfo t;
    ...
    fprintf (out, " (ADDRLEN INTEGER \"socklen_t\"\n");
{ struct addrinfo t;
